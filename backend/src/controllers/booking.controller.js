...
exports.createBooking = async (req, res, next) => {
  try {
    ...
    // Check if it's a coach booking
    if (bookingData.type !== 'facility_booking') {
      ...
      // For facility_booking, check if coachId is a valid User (the owner)
      const owner = await User.findById(bookingData.coachId);
      if (!owner) {
        return res.status(400).json({
          success: false,
          message: 'Facility owner not found'
        });
      }
    }
