...
const AddProductPage = () => {
  ...
  const loadStoreProfile = async () => {
    try {
      ...
      if (response.data?.success && response.data?.data?._id) {
        ...
        const errorMessage = response.data?.message || 'Store profile not found';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!storeId) {
      setError('Store ID not found. Please try again.');
      return;
    }
    ...
    try {
      ...
      if (formData.images.length > 0) {
        ...
        // For now, send product data first, then update with images
        const response: any = await ApiService.stores.addProduct(storeId, productData);
        
        if (response.data?.success) {
          // If images need to be uploaded separately, handle here
  ...
  return (
    <StoreLayout title="Add Product - TeamUp India" description="Add a new product to your store">
